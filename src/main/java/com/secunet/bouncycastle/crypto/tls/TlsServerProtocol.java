    ...
    extends TlsProtocol
{
        ...
        throws IOException
    {
        ...
        Hashtable offeredClientExtensions = readExtensions(buf);
       
        /*
         * RFC 5077 3.4. When presenting a ticket, the client MAY generate and
         * include a Session ID in the TLS ClientHello.  If the server accepts the
         * ticket and the Session ID is not empty, then it MUST respond with the
         * same Session ID present in the ClientHello.  This allows the client to
         ...
         * particular value when validating the ticket.
         * If a ticket is presented by the client, the server MUST NOT attempt to
         * use the Session ID in the ClientHello for stateful session resumption.
         */
        byte[] offeredSessionTicket = TlsExtensionsUtils.getSessionTicketExtension(offeredClientExtensions);
        if(offeredSessionTicket != null && offeredSessionTicket.length > 0)
        {
            this.tlsSession = tlsServer.getResumableSession(requestedClientSessionID, offeredSessionTicket);
        }
        else
        {
         ...
         * resumption request), this vector [offeredCipherSuites] MUST include at least the cipher_suite from that
         * session.
         */
        if (this.resumedSession)
        {
        ...
        /*
         * RFC 5246 7.4.1.2. If the session_id field is not empty (implying a session
         * resumption request), it [offeredCompressionMethods] MUST include the compression_method from that session.
         */
        if (this.resumedSession)
        {
        ...
        if (this.resumedSession)
        {
            Hashtable newClientExtensions = offeredClientExtensions;
        ...
        {
            this.clientExtensions = offeredClientExtensions;
        }
